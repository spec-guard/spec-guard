...
const COMMANDS = {
  ...
  coordinate: {
    summary: 'Orchestrate several independent front-loop instances in parallel: worktree per front, async HITL, sequential merge behind a test gate.',
    usage: 'specguard coordinate <plan|start|report|status|watch|answer|merge|resolve-merge|finish|list> [options]',
    subcommands: ['plan', 'start', 'report', 'status', 'watch', 'answer', 'merge', 'resolve-merge', 'finish', 'list'],
    flags: [
      '--run <id>            the run to act on',
      '--lane <id>[,id2,..]  one or more lane ids',
      '--root <path>         repo root (default: cwd)',
      '--file <f>            plan: JSON array of lane defs | answer: read the answer from a file',
      '--run-id <id>         plan: pin the run id instead of generating one',
      '--base-branch <b>     plan: branch fronts fork from (default: current branch)',
      '--dry-run             plan: validate only, write nothing',
      '--declared-paths <g>  report: comma-separated glob(s) this lane actually touches',
      '--status <s>          report: pending|running|blocked|verified|merging|merged|failed|aborted',
      '--question <t>        report: the blocking question, with --status blocked',
      '--urgency <l>         report: low|normal|high (default normal)',
      '--note <t>            report: freeform note appended to the lane',
      '--json                status: machine-readable output',
      '--refresh             status: re-run the overlap check before reporting',
      '--interval <sec>      watch: poll interval in seconds (default 10)',
      '--text <t>            answer: the human answer',
      '--test-cmd <cmd>      merge: override the resolved test-gate command',
      '--action <a>          resolve-merge: retry|abort',
      '--force               finish: proceed even with non-terminal lanes; resolve-merge abort: finalize as aborted even though a module could not be auto-reverted',
      '--purge-worktrees     finish: remove worktrees for terminal lanes',
      '--keep-branches       finish: keep branches when purging worktrees',
      '--active              list: only non-terminal runs',
    ],
  },
};
