...
| 2 | **IP knowledge base** — troubleshooting, action plans, audits, internal rationale, internal standards, agent templates, **credentials** | `${ipDir}/` (default `.private/`) | NO | **no — harness-agnostic** |
...
**read by humans and by every AI agent**; it must live in a neutral `${ipDir}/`, so a repo using
...
> (→ `${ipDir}/`), or **how one AI tool plugs in** (→ that agent's dir, usually auto-generated)?

- Architecture, API contract, schema, ADR (the decision), runbook a client could need → `docs/`.
- Troubleshooting playbook, internal audit, agent checklist, credentials, "here's the trick" → `${ipDir}/`.
- A formal *decision* goes in `docs/reference/decisions/` (ADR); the *messy reasoning / rejected
  options* behind it can live in `${ipDir}/docs/` and be linked from the ADR as "internal rationale".

## The golden rule (cross-reference direction)

> **Internal MAY reference `docs/`. `docs/` MUST NOT reference internal.**

"Internal" = the IP knowledge base (`${ipDir}/`) **and** any per-agent dir (`.claude/`, `.codex/`,
...
> Note for linters: a *hyperlink* whose target points into `${ipDir}/` or an agent dir (at any
...
concept/decision/contract → `docs/`; the execution heuristics / gotchas / checklists → `${ipDir}/`;
link from internal → deliverable (never the reverse).

## Credentials

Secrets live only in `${ipDir}/credentials/` (and per-module env under it). Never in `docs/`,
