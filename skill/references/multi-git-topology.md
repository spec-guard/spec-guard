...
│   ├── .git/
│   ├── .gitignore               ← excludes ${privateDir}/ .env* agent-dirs graphify-out/ (never ships)
│   └── ${privateDir}/                ← IP knowledge base: local + in backup, gitignored in the deliverable
...
3. **IP is gitignored per-module but captured at the root.** The IP knowledge base (`${privateDir}/`),
...
4. **Ride-along dirs.** Some folders have no own `.git`; they are not separate deliverables and
   ...
   (`${privateDir}/`, agent dirs, `.env*`, generated artifacts) **before** `git init` + push, or IP leaks.
