...
|---|---|---|---|---|---|---|---|---|
| Claude Code | Complete | Repo skill + home fallback | `.claude/skills/spec-guard/`, `.claude/commands/`, `CLAUDE.md` block | `~/.claude/skills/spec-guard/`, `~/.claude/hooks/spec-guard/`, `~/.claude/settings.json`, statusline | `SessionStart` hook | `SessionStart`, `Stop`, statusline | `/spec`, `/spec:orient`, `/spec:write`, `/spec:verify`, `/spec:sync`, `/spec:commit`, `/spec:status`, `/spec:coordinate` | Machine hooks require `setup` or `init --with-global` |
| Codex | Partial | Home skill | `AGENTS.md` block | `~/.codex/skills/spec-guard/`, `~/.codex/hooks/spec-guard/`, `~/.codex/hooks.json` | `SessionStart` hook | `SessionStart`, `Stop` | Natural language only (incl. `specguard coordinate`) | No file-backed slash commands or statusline |
| GitHub Copilot | Instructional | Repo skill | `.github/skills/spec-guard/`, `.github/prompts/spec-*.prompt.md`, `.github/copilot-instructions.md` block | Nothing | Project instructions | None wired | Prompt files such as `#spec-orient.prompt.md`, `#spec-coordinate.prompt.md` | No spec-guard lifecycle hook |
| Gemini CLI | Complete | Repo extension | `.gemini/extensions/spec-guard/` including skill, commands, hooks, `gemini-extension.json`, `GEMINI.md` block | Nothing | Gemini extension hooks | Repo-scoped extension hooks | `/spec`, `/spec:orient`, `/spec:write`, `/spec:verify`, `/spec:sync`, `/spec:commit`, `/spec:status`, `/spec:coordinate` | Extension is repo-scoped, not workstation-wide |
| opencode / OpenWork | Instructional | Repo skill | `.opencode/skills/spec-guard/`, `.opencode/commands/spec-*.md`, `AGENTS.md` block | Nothing | Project memory | None wired | `/spec` (umbrella), `/spec-orient`, `/spec-write`, `/spec-verify`, `/spec-sync`, `/spec-commit`, `/spec-status`, `/spec-coordinate` | No spec-guard lifecycle hook |
